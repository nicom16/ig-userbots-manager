...
const Instauto = async (db, browser, options) => {
  ...
  function checkReachedFollowedUserDayLimit() {
    if (getNumFollowedUsersThisTimeUnit(dayMs) >= maxFollowsPerDay) {
      logger.log('Have reached daily follow/unfollow limit, stopping');
      return true;
    } else {
      return false;
    }
  ...
  async function checkReachedFollowedUserDayLimitOriginal() {
    if (getNumFollowedUsersThisTimeUnit(dayMs) >= maxFollowsPerDay) {
  ...
  function checkReachedLikedUserDayLimit() {
    if (getNumLikesThisTimeUnit(dayMs) >= maxLikesPerDay) {
      logger.log('Have reached daily like rate limit, stopping');
      return true;
    } else {
      return false;
    }
  ...
  async function checkReachedLikedUserDayLimitOriginal() {
    if (getNumLikesThisTimeUnit(dayMs) >= maxLikesPerDay) {
  ...
  async function navigateToUser(username) {
    ...
    if (status === 200) {
      ...
      await page.screenshot({ path: './error.jpg', type: 'jpeg', quality: 30 });
  ...
  async function likeUserFollowers(username, {
    maxFollowsPerUser = 5, skipPrivate = true, enableLikeImages, likeImagesMin, likeImagesMax } = {}) {
    ...
    //await throttle();
    // Throttle
    let check = checkReachedLikedUserDayLimit();
    ...
    for await (const followersBatch of getFollowersOrFollowingGenerator({ userId, getFollowers: true })) {
      ...
      for (const follower of followersBatch) {
        //await throttle();
        // Throttle
	check1 = checkReachedFollowedUserDayLimit();
        check2 = checkReachedLikedUserDayLimit();
	  ...
	  // Checking reached limits
	  if (count == fixedMaxFollows) {
	    console.log(`Followed ${fixedMaxFollows} users, sleeping!`);
	    return true;
	  }
        } catch (err) {
  ...
  async function processUserFollowers(username, followers, {
    maxFollowsPerUser = 5, skipPrivate = false, enableLikeImages, likeImagesMin, likeImagesMax, fixedMaxFollows } = {}) {
    const enableFollow = maxFollowsPerUser > 0;
    ...
    //await throttle();
    // Throttle
    let check1 = checkReachedFollowedUserDayLimit();
    let check2 = checkReachedLikedUserDayLimit();
    if (check1 || check2) return { sleep: false, followers: followers };
    ...
    if (!followers) {
	  ...
	  check1 = checkReachedFollowedUserDayLimit();
          ...
          try {
            if (enableFollow && numFollowedForThisUser >= maxFollowsPerUser) {
              logger.log('Have reached followed limit for this user, stopping');
              return { sleep: false, followers: followers };
            }
	    ...
	    if (count == fixedMaxFollows) {
	      console.log(`Followed ${fixedMaxFollows} users, sleeping!`);
	      return { sleep: true, followers: followers };
	    }
          } catch (err) {
            logger.error(`Failed to process follower ${follower}`, err);
            await takeScreenshot();
  ...
  async function originalProcessUserFollowers(username, {
  ...
  async function processUsersFollowers({ usersToFollowFollowersOf, maxFollowsTotal = 150, skipPrivate, enableFollow = true, enableLikeImages = false, likeImagesMin = 1, likeImagesMax = 2, fixedMaxFollows }) {
    ...
    for (const username of usersToFollowFollowersOfSliced) {
      try {
        await processUserFollowers(username, { maxFollowsPerUser, skipPrivate, enableLikeImages, likeImagesMin, likeImagesMax, fixedMaxFollows });
  ...
  await tryPressButton(await page.$x('//button[contains(text(), "Allow all cookies")]'), 'Accept cookies dialog 2 button 2', 10000);
  ...
  async function sendDm(username, message) {
    await navigateToUserWithCheck(username);
    let messageButton = await page.$x('//div[contains(@role, "button") and contains(., "Message")]');

    if (!messageButton[0]) {
      console.log("No message button available");
      return false;
    }

    logger.log(`Messaging ${username}`);

    await messageButton[0].click();
    await sleep(3000);

    let textArea = await page.$x('//textarea[contains(@placeholder, "Message...")]');
    await textArea[0].type(message);
    await sleep(3000);
    
    let sendButton = await page.$x('//div[contains(@role, "button") and contains(., "Send")]');
  ...
  async function processUserForStories(username, followers, iterations) {
    if (!followers) {
      const { id: userId } = await navigateToUserAndGetData(username);
      ...
      for await (const followersBatch of getFollowersOrFollowingGenerator({ userId, getFollowers: true })) {
        ...
        for (const follower of followersBatch) {
          let viewed = await viewStory(follower);
  ...
  async function processUserForDms(username, followers, messages, iterations) {
    if (!followers) { 
      const { id: userId } = await navigateToUserAndGetData(username);
	  ...
	  let wasSent = await sendDm(follower, messages[index]);

	  if (wasSent) {
            iterations--;
	    followers.shift();
	    if (iterations > 0) await sleep(60 * 1000);
	  }
  ...
  async function graphqlQueryFollowers(pages, { queryHash, getResponseProp, graphqlVariables: graphqlVariablesIn }) {
    const graphqlUrl = `${instagramBaseUrl}/graphql/query/?query_hash=${queryHash}`;
  ...
  async function processUser(username, pages) {
    const { id: userId } = await navigateToUserAndGetData(username);

    return await graphqlQueryFollowers(pages, {
      getResponseProp: (json) => json.data.user[true ? 'edge_followed_by' : 'edge_follow'],
      graphqlVariables: { id: userId },
      queryHash: true ? '37479f2b8209594dde7facb0d904896a' : '58712303d941c6855d4e888c5f0cd22f',
    });
  }
